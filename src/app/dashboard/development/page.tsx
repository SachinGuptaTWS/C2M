...
// 1.5px stroke, 24x24 viewBox, optically balanced.
...
const Icons = {
  ...
  User: (props: any) => <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" {...props}><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>,
...
function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const navLinks = [
    ...
    { name: 'Development', href: '/dashboard/development', icon: Icons.Code, active: true },
    { name: 'Algorithms', href: '/dashboard/dsa', icon: Icons.Cpu },
  ];
...
function DevelopmentContent() {
  ...
  const fetchVideos = async () => {
    try {
      const response = await fetch('/api/videos?category=DEVELOPMENT');
                        ...
                        <div className="flex items-start justify-between gap-6 px-1">
                             <div className="space-y-2">
                                <div className="flex items-center gap-2 mb-1">
                                    <span className="px-1.5 py-0.5 rounded-md bg-white/10 text-zinc-300 text-[10px] font-medium border border-white/5 uppercase tracking-wider">
            ...
            <div className="space-y-1 max-h-[calc(100vh-10rem)] overflow-y-auto pr-1">
                ...
                {videos.length === 0 && (
                    <div className="text-center py-8 text-zinc-500">
                        No development videos available yet.
                    </div>
                )}
            </div>
        </div>
